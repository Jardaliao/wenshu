...
// 案件类型
export const ajlxMap = Object.fromEntries(dic.ajlx.map(({ code, name }) => [code, name]));
// 法院层级
export const fycjMap = Object.fromEntries(dic.fycj.map(({ code, name }) => [code, name]));
// 文书类型
export const wslxMap = Object.fromEntries(dic.wslx.map(({ code, name }) => [code, name]));
// 案例等级
export const aldjMap = Object.fromEntries(dic.aldj.map(({ code, name }) => [code, name]));
// 公开类型
export const gklxMap = Object.fromEntries(dic.gklx.map(({ code, name }) => [code, name]));
// 结案方式
export const jaMap = Object.fromEntries(dic.ja.map(({ code, name }) => [code, name]));
// 案由
dic.ayMap = {};
dic.ayValueMap = {};
dic.ayChildMap = {};// 有层级结构的案由map，key是parent，value是子级的array
dic.ayNameKeyMap = {};// key是案由名称，value是案由信息，高级检索智能提示不返回名称对应的s*所用
for (let item of dic.ay) {
    dic.ayMap[item["id"]] = item["text"];
    dic.ayValueMap[item["id"]] = item;
    dic.ayNameKeyMap[item["text"]] = item;
    if (dic.ayChildMap[item["parent"]] == null) {
        dic.ayChildMap[item["parent"]] = [];
    }
...
for (var key in spcxArr) {
    ...
    if (typeof values !== "string") {
        for (var i = 0; i < values.length; i++) {
            dic.spcxMap[values[i]["id"]] = values[i]["text"];
        }
    }
}
